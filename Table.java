...
//      case BPTREE_MAP  -> new BpTreeMap <> (KeyType.class, Comparable [].class);
    ...
    public Table select (KeyType keyVal)
    {
        ...
        // Creating file name to store the output of indexed select operation
        String tableName = "indexed_select";
        // Creating a FileList instance
        FileList fileList = new FileList(tableName, rows.size());
        // Iterating through rows of the indexed join and adding each tuple to the FileList
         for(Comparable[] tuple: rows) 
         {
            //addding the tuple
            fileList.add(tuple);
         }
         // A debug statement to print the 0th tuple which is the first record
         fileList.get(0);
    ...
    public Table i_join (String attributes1, String attributes2, Table table2)
     {
         out.println ("RA> this is i_join" + name + ".i_join (" + attributes1 + ", " + attributes2 + ", "
                                                + table2.name + ")");
        //  T O   B E   I M P L E M E N T E D  - Project 2

        //Retriving the attributes specified in the function call
         var attrs_1 = attributes1.split (" ");
         var attrs_2 = attributes2.split (" ");
         /* Bonus to be removed   
         */
         ...
         // Indexing on the join column of table2
         for (Comparable[] table2_tup : table2.tuples) 
        	 index.put (new KeyType (table2_tup[table2.col(attrs_2[0])]), table2_tup);

        // Joining the tables using the indexed join algorithm
       // Creating a list to store the resulting rows after performing the indexed join
         List <Comparable []> resultRows = new ArrayList <> ();
         // Iterating through all the rows of Table2
         for (Comparable[] table1_tup : tuples) 
         {
             Comparable[] table2_tup = index.get(new KeyType(table1_tup[col(attrs_1[0])]));
             // Checking if a matching row was found in the index for Table 2
             if (table2_tup != null) 
             {
                //Concatinating table1 and table 2 tuples
                Comparable[] joinedRow = concat(table1_tup,table2_tup);
                //Adding the joined tuples to the resultant rows
                resultRows.add(joinedRow);
             }
         }
         // Creating file name to store the output of i_join operation
         String tableName = "i_join";
         // Creating a FileList instance
         FileList fileList = new FileList(tableName, resultRows.size());
         // Iterating through resultRows of the i_join and adding each tuple to the FileList
         for(Comparable[] tuple: resultRows) 
         {
            //addding the tuple
            fileList.add(tuple);
         }
         // A debug statement to print the 3rd tuple which gives the 4th record
         fileList.get(3);
         return new Table (name + count++, concat (attribute, table2.attribute),concat (domain, table2.domain), key, resultRows);
     } // i_join
